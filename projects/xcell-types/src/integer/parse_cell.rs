...
impl IntegerDescription {
    ...
    fn parse_i8(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_i8() {
            Some(s) => Ok(XCellValueKind::Integer8(s)),
            None => syntax_error(format!("{int} 无法转化为 i8 类型")),
        }
    }
    fn parse_i16(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_i16() {
            Some(s) => Ok(XCellValueKind::Integer16(s)),
            None => syntax_error(format!("{int} 无法转化为 i16 类型")),
        }
    }
    fn parse_i32(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_i32() {
            Some(s) => Ok(XCellValueKind::Integer32(s)),
            None => syntax_error(format!("{int} 无法转化为 i32 类型")),
        }
    }
    fn parse_i64(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_i64() {
            Some(s) => Ok(XCellValueKind::Integer64(s)),
            None => syntax_error(format!("{int} 无法转化为 i64 类型")),
        }
    }
    fn parse_u8(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_u8() {
            Some(s) => Ok(XCellValueKind::Unsigned8(s)),
            None => syntax_error(format!("{int} 无法转化为 u8 类型")),
        }
    }
    fn parse_u16(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_u16() {
            Some(s) => Ok(XCellValueKind::Unsigned16(s)),
            None => syntax_error(format!("{int} 无法转化为 u16 类型")),
        }
    }
    fn parse_u32(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_u32() {
            Some(s) => Ok(XCellValueKind::Unsigned32(s)),
            None => syntax_error(format!("{int} 无法转化为 u32 类型")),
        }
    }
    fn parse_u64(&self, cell: &DataType) -> XResult<XCellValueKind> {
        let int = self.parse_value(cell)?;
        match int.to_u64() {
            Some(s) => Ok(XCellValueKind::Unsigned64(s)),
