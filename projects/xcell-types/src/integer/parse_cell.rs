...
impl IntegerDescription {
    ...
    pub fn parse_value(&self, cell: &Data) -> XResult<BigInt> {
        match cell {
            ...
            Data::Float(f) => match BigInt::from_f64(*f) {
                Some(o) => Ok(o),
                None => syntax_error(format!("浮点数 `{}` 无法解析为整数类型", f)),
            },
            Data::String(s) => match BigInt::from_str(s) {
                Ok(o) => Ok(o),
    ...
    fn parse_i8(&self, cell: &Data) -> XResult<XCellValue> {
    ...
    fn parse_i16(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_i16() {
    ...
    fn parse_i32(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_i32() {
    ...
    fn parse_i64(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_i64() {
    ...
    fn parse_u8(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_u8() {
    ...
    fn parse_u16(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_u16() {
    ...
    fn parse_u32(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_u32() {
    ...
    fn parse_u64(&self, cell: &Data) -> XResult<XCellValue> {
        let int = self.parse_value(cell)?;
        match int.to_u64() {
