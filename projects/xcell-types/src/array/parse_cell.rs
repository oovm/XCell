...
impl ArrayDescription {
    ...
    fn parse_vec2(&self, cell: &Data) -> XResult<XCellValue> {
        let vec = self.parse_value(cell)?;
        Ok(XCellValue::Vector2(fill_array(&vec)))
    }
    fn parse_vec3(&self, cell: &Data) -> XResult<XCellValue> {
        let vec = self.parse_value(cell)?;
        Ok(XCellValue::Vector3(fill_array(&vec)))
    }
    fn parse_vec4(&self, cell: &Data) -> XResult<XCellValue> {
        let vec = self.parse_value(cell)?;
        Ok(XCellValue::Vector4(fill_array(&vec)))
    }
    fn parse_quaternion4(&self, cell: &Data) -> XResult<XCellValue> {
        let vec = self.parse_value(cell)?;
