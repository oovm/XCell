...
use crate::{ProjectConfig, project::Generator};
use oak_toml;
...
impl GeneratorConfig {
    ...
    pub fn from_file(path: &Path) -> Result<Self, String> {
        ...
        file.read_to_string(&mut content).map_err(|e| format!("无法读取配置文件: {:?}", e))?;

        oak_toml::from_str(&content).map_err(|e| format!("配置文件格式错误: {:?}", e))
    }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for (index, product) in self.products.iter().enumerate() {
            if product.output_dir.is_empty() {
                return Err(format!("产物 {} 的输出目录不能为空", index));
            }
        }
