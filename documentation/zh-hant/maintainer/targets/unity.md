...
> ⚠️ **注意**：Unity 程式碼生成器当前奮於停用狀態，正在重构中。以下文件僅供參考，功能可能不可用。

XCell 提供了與 Unity 引擎的深度集成，支援生成 C# 程式碼、二進位元資料檔案等多种格式。

## 当前狀態

Unity 程式碼生成器 (`unity`) 当前奮於停用狀態，原因如下：
...
在 Unity 程式碼生成器重新啟用之前，您可以考虑以下替代方案：

1. **使用 JSON 資料格式**：通过 [JSON](json.md) 生成器匯出資料，在 Unity 中使用 `JsonUtility` 或 `Newtonsoft.Json` 剖析
2. **使用 TypeScript 生成器**：通过 [TypeScript](typescript.md) 生成類型定義，手勁编農 C# 類別
3. **使用 Dejavu 範本**：通过 [Dejavu 範本引擎](../architecture/index.md#程式碼生成) 自訂程式碼生成

## 設定選項（參考）

在 `ProjectSettings.toml` 檔案中，Unity 集成設定位元於 `[unity]` 部分：
...
Unity 程式碼生成器正在进行重构，以支援更好的類型系統和程式碼生成架构。预计屆在未来版本中重新啟用。

### 如何获取最新狀態？

请茲注專案更新日誌或查看 `backends/xcell-generator/src/codegen/unity/` 目錄下的程式碼號更。
