...
XCell は Vue フレームワークとの深い統合を提供し、TypeScript コード、JSON データファイルなど複数のフォーマットの生成をサポートしています。

## 設定オプション

`XCell.toml` ファイルで、Vue 統合設定は `[vue]` セクションにあります：
...
5. **Vue Composition API を使用**：生成された Composition API 関数でデータ読み込みと状態管理を簡素化

## サンプルプロジェクト

XCell は Vue サンプルプロジェクトを提供し、実際のプロジェクトで XCell を使用する方法を示しています：
...
サンプルプロジェクトを通じて、Vue での XCell のベストプラクティスを素早く理解できます。
