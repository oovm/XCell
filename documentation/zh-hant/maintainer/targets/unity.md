...
> ⚠️ **注意**：Unity 程式碼產生器當前處於停用狀態，正在重构中。以下文件僅供參考，功能可能不可用。
...
2. **使用 TypeScript 生成器**：通过 [TypeScript](typescript.md) 生成類型定義，手動编寫 C# 类
...
Unity 程式碼產生器正在进行重构，以支援更好的類型系統和程式碼產生架构。预计將在未来版本中重新啟用。
