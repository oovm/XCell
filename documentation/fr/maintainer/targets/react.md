...
XCell fournit une intégration profonde avec le framework React, supportant la génération de code TypeScript, les fichiers de données JSON et d'autres formats.

## Options de configuration

Dans le fichier `XCell.toml`, la configuration d'intégration React se trouve dans la section `[react]` :
...
function BuffList() {
    ...
    if (!manager) {
        return <div>Échec du chargement des données</div>;
    }
...
- Vérifier si le chemin du projet React est correct
...
5. **Utiliser React Hooks** : Utiliser les React Hooks générés pour simplifier le chargement des données et la gestion de l'état

## Projet exemple

XCell fournit un projet exemple React démontrant comment utiliser XCell dans des projets réels :
...
Grâce au projet exemple, vous pouvez apprendre rapidement les bonnes pratiques de XCell dans React.
